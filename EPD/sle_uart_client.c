...
errcode_t sle_uart_client_send_data(const uint8_t *data, uint16_t len, uint16_t conn_id)
{
    ...
    param->data = send_buf;
    param->data_len = len;
    param->type = SSAP_PROPERTY_TYPE_VALUE; // 确保类型正确
    
    errcode_t ret = ssapc_write_req(0, g_sle_uart_conn_id[conn_id], param);
    osal_printk("send_data:%s\r\n",param->data);
